...
vec3 light(0.0, 1.1, 1.0); // cornell light 
    ...
    for(int x = 0; x < window.width; x++) {
		for(int y = 0; y < window.height; y++) {
			...
			if(!isinf(rt_int.distanceFromCamera)){
                float scale = pixelBrightness(rt_int, 300);
                // scale = (scale > 0.3) ? scale : 0.3;
                if(rt_int.intersectedTriangle.colour.name != ""){
                    ModelTriangle t = rt_int.intersectedTriangle;
                    //get texture map
                    TextureMap texture = textures[t.colour.name];

                    float t_x = ((1 - rt_int.u - rt_int.v) * t.texturePoints[0].x + rt_int.u * t.texturePoints[1].x + rt_int.v * t.texturePoints[2].x);

					float t_y = ((1 - rt_int.u - rt_int.v) * t.texturePoints[0].y + rt_int.u * t.texturePoints[1].y + rt_int.v * t.texturePoints[2].y);
...
function<void(vector<ModelTriangle>, float, int, unordered_map<string, TextureMap>, DrawingWindow &)> drawing = draw_rasterise;
...
int main(int argc, char *argv[]) {
	...
	// vector<ModelTriangle> t_sphere = load_obj("models/sphere.obj", 0.4, load_mtl("models/cornell-box.mtl"));
